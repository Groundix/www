...
const Navbar = (props) => {
  ...
  // Проверяем, что props.state и props.state.friendList определены
  if (!props.state || !props.state.friendList) {

  } else {

    // Отображаем список друзей
    let friendListElement = props.state.friendList.map((f) => (
      <FriendList key={f.id} name={f.name} id={f.id} img={f.img} />
    ));
    return (
